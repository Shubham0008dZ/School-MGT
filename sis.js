...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // DYNAMIC USER NAME REPLACEMENT (Top Right)
    const topRightSpans = document.querySelectorAll('.top-right span');
    if(topRightSpans.length > 0) {
        topRightSpans[0].innerHTML = `👤 Welcome, <b>${activeUser.empName}</b>`;
    }

    if (!isSA && !userRights.some(r => r.startsWith("SIS_"))) { window.location.href = 'index.html'; return; }

    // AGGRESSIVE DOM REMOVAL FOR UNAUTHORIZED ACTIONS
    if (!isSA) {
    ...
    if(btnLogout) {
        btnLogout.addEventListener('click', () => {
            customConfirm("Are you sure you want to logout?", () => { localStorage.removeItem('erp_active_user'); window.location.href = 'login.html'; });
        });
    }
    ...
    function updateNextRegNo() {
        let maxReg = 0;
        appData.forEach(s => { let num = parseInt(s.regNo, 10); if(!isNaN(num) && num > maxReg) maxReg = num; });
        ...
        fetch(scriptURL)
            ...
            .then(res => {
                if(res.status === "Success") {
                    ...
                    loadSetupDropdowns(); renderTable(appData); renderDashboard(); updateNextRegNo(); 
                } else tbody.innerHTML = `<tr><td colspan="8" style="color:red;">Error: ${res.message}</td></tr>`;
    ...
    function loadSetupDropdowns() {
        ...
        function fillSelect(id, array, isObj = false) {
            ...
            if(!el) return; el.innerHTML = '<option value="">Select</option>';
    ...
    function renderTable(dataToRender) {
        ...
        dataToRender.forEach(student => {
            ...
            // STRICT RBAC CHECK FOR ROW BUTTONS
            let btnHTML = "";
    ...
    function calculateAgeForGraph(dobStr) {
        if(!dobStr) return "N/A"; let diff = new Date() - new Date(dobStr); return Math.floor(diff / 31557600000); 
    }
    ...
    function renderDashboard() {
        ...
        appData.forEach(s => {
            counts.class[s.studentClass || 'NA'] = (counts.class[s.studentClass || 'NA'] || 0) + 1; counts.blood[s.bloodGroup || 'NA'] = (counts.blood[s.bloodGroup || 'NA'] || 0) + 1;
            counts.cat[s.category || 'NA'] = (counts.cat[s.category || 'NA'] || 0) + 1; counts.rel[s.religion || 'NA'] = (counts.rel[s.religion || 'NA'] || 0) + 1; counts.house[s.house || 'NA'] = (counts.house[s.house || 'NA'] || 0) + 1;
    ...
    window.editStudent = function(s) {
        ...
        document.getElementById('formTitle').innerText = "Edit Student Profile"; document.getElementById('saveSubmitBtn').innerText = "Update Record in DB";
        document.getElementById('saveSubmitBtn').style.background = "#f39c12"; document.getElementById('editMode').value = "true"; formTabs[0].click(); 

        setVal('regNo', s.regNo); document.getElementById('regNo').readOnly = true; setVal('adminDate', s.adminDate ? new Date(s.adminDate).toISOString().split('T')[0] : '');
        setVal('studentFirstName', s.studentFirstName || s.studentName); setVal('studentLastName', s.studentLastName); setVal('primaryEmail', s.primaryEmail);
        setVal('dob', s.dob ? new Date(s.dob).toISOString().split('T')[0] : ''); if(s.dob) document.getElementById('dob').dispatchEvent(new Event('change'));
        ...
        setVal('corrAddress', s.corrAddress); setVal('corrCity', s.corrCity); setVal('corrState', s.corrState); setVal('corrCountry', s.corrCountry); setVal('corrPincode', s.corrPincode);
        setVal('permAddress', s.permAddress); setVal('permCity', s.permCity); setVal('permState', s.permState); setVal('permCountry', s.permCountry); setVal('permPincode', s.permPincode);
    };

    window.deleteStudent = function(regNo) {
        customConfirm(`Are you sure you want to delete Reg No: ${regNo} from Database?`, () => {
            fetch(scriptURL, { method: 'POST', body: JSON.stringify({ action: "delete", regNo: regNo }) }).then(res => res.json())
            .then(data => { if(data.status === "Success") { customAlert(data.message || "Deleted from DB!"); syncWithDatabase(); } });
        });
    };

    document.getElementById('admissionForm').addEventListener('submit', function(e) {
        e.preventDefault(); const submitBtn = document.getElementById('saveSubmitBtn'); const isEdit = document.getElementById('editMode').value === "true";
    ...
    document.getElementById('btn-back-to-profiles').addEventListener('click', () => {
        document.querySelectorAll('.app-module').forEach(m => m.classList.remove('active-module')); document.getElementById('module-profiles').classList.add('active-module');
    });
