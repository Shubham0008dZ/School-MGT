...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTable(dataToRender) {
        ...
        dataToRender.forEach(student => {
            ...
            tr.innerHTML = `<td>${student.regNo || '-'}</td><td>${student.studentClass || '-'}</td><td><a href="#" class="student-ledger-link" onclick="openLedger('${student.regNo}')" title="View Fee Ledger">${student.studentFirstName || student.studentName || '-'}</a></td><td>${student.gender || '-'}</td><td>${student.category || '-'}</td><td>${student.bloodGroup || '-'}</td><td>${student.house || '-'}</td><td>${btnHTML}</td>`;
