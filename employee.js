// ============================================================
// CUSTOM ALERT / CONFIRM (original - unchanged)
// ============================================================
...
// ============================================================
// NETWORK DIAGNOSTICS (original - unchanged)
// ============================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ---- SIDEBAR TOGGLE (original - unchanged) ----
    ...
    // ---- AUTH & SESSION (original - unchanged) ----
    ...
    // ---- NAVBAR LOGO/NAME (original - unchanged) ----
    try {
    ...
    // ---- SESSION VERIFY (original - unchanged) ----
    ...
    // ---- RIGHTS CHECK (original - unchanged) ----
    ...
    // ---- DATA STORES (original - unchanged) ----
    let allEmployees = [];
    ...
    // ---- MODULE NAVIGATION (original - unchanged) ----
    ...
    // ---- TAB ORDER (for Previous / Save & Next navigation) ----
    const tabOrder = ['tab-school', 'tab-family', 'tab-documents', 'tab-education', 'tab-experience', 'tab-financial'];

    function switchToTab(tabId) {
        formTabs.forEach(t => t.classList.remove('active'));
        tabContents.forEach(c => c.classList.remove('active'));
        const targetBtn = document.querySelector(`.erp-tab-btn[data-target="${tabId}"]`);
    ...
    function getCurrentTabId() {
        const active = document.querySelector('.erp-tab-btn.active');
        return active ? active.getAttribute('data-target') : tabOrder[0];
    }

    function goNextTab() {
        const current = getCurrentTabId();
        const idx = tabOrder.indexOf(current);
        if(idx < tabOrder.length - 1) switchToTab(tabOrder[idx + 1]);
    }

    function goPrevTab() {
        const current = getCurrentTabId();
        const idx = tabOrder.indexOf(current);
        if(idx > 0) switchToTab(tabOrder[idx - 1]);
    }

    // Wire up all "Save & Next" buttons (tab footers)
    document.querySelectorAll('.emp-btn-save-next').forEach(btn => {
    ...
    // Wire up all "Previous" buttons (tab footers)
    document.querySelectorAll('.emp-btn-prev').forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            goPrevTab();
        });
    ...
    // Wire up all "Cancel" buttons (tab footers) — go back to list
    document.querySelectorAll('.emp-btn-cancel').forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
    ...
    // Wire up all "Save & Close" buttons except the final submit (financial tab uses submit)
    document.querySelectorAll('.emp-btn-save-close').forEach(btn => {
        if(btn.id === 'btnSaveEmp') return; // final save handled by form submit
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            // Trigger form submit
    ...
    // ============================================================
    // CROPPER & PHOTO UPLOAD LOGIC (original - unchanged)
    // ============================================================
    let cropper = null;
    ...
    // ============================================================
    // SIGNATURE UPLOAD (original - unchanged)
    // ============================================================
    ...
    // ============================================================
    // AGE CALCULATOR (original - unchanged)
    // ============================================================
    document.getElementById('empDob')?.addEventListener('change', function() {
        ...
        document.getElementById('empAge').value = `${years} years ${months} month and ${days} days`;
    });

    // ============================================================
    // FILTER AND SEARCH LOGIC (original - unchanged)
    // ============================================================
    ...
    // ============================================================
    // RENDER EMPLOYEE TABLE (original - unchanged)
    // ============================================================
    function renderEmployeesTable(dataList) {
        ...
        activeEmps.forEach((emp, idx) => {
            ...
            tbody.innerHTML += `<tr>
                ...
                <td><b>${emp.empId}</b></td><td>${emp.empSalutation || ''}</td><td>${emp.empName}</td>
                <td>${emp.empDept || '-'}</td><td>${emp.empDesig || '-'}</td><td>${emp.empMobile || '-'}</td>
                <td>${emp.empBio || '-'}</td><td>${actionHtml}</td>
            </tr>`;
        });
    }

    // ============================================================
    // ADD / EDIT FORM LOGIC (original - unchanged)
    // ============================================================
    ...
    document.getElementById('chkSameAddr')?.addEventListener('change', function() {
        if(this.checked) {
            setVal('empPermAdd', getVal('empCorrAdd'));
            setVal('empPermCity', getVal('empCorrCity'));
            setVal('empPermState', getVal('empCorrState'));
            setVal('empPermCountry', getVal('empCorrCountry'));
            setVal('empPermPin', getVal('empCorrPin'));
        }
        let sw = this.closest('.switch-custom');
        if(sw) { let txt = sw.querySelector('.switch-text'); if(txt) txt.textContent = this.checked ? 'YES' : 'NO'; }
    });

    // All switch-custom YES/NO text update
    document.querySelectorAll('.switch-custom input[type="checkbox"]').forEach(chk => {
    ...
    // ============================================================
    // EDIT EMPLOYEE (original - unchanged)
    // ============================================================
    window.editEmp = function(e) {
        ...
        setVal('empGender', e.empGender); setVal('empBlood', e.empBlood);
        setVal('empDob', e.empDob ? new Date(e.empDob).toISOString().split('T')[0] : '');
        setVal('empAge', e.empAge);
        ...
        setVal('empBio', e.empBio); setVal('empSubjects', e.empSubjects);
        setVal('empConfDate', e.empConfDate ? new Date(e.empConfDate).toISOString().split('T')[0] : '');
        setVal('empRetireDate', e.empRetireDate ? new Date(e.empRetireDate).toISOString().split('T')[0] : '');
        ...
        if(e.empPhotoBase64 && e.empPhotoBase64 !== DEFAULT_AVATAR) { togglePhotoButtons(true); } else { togglePhotoButtons(false); }
    ...
    // ============================================================
    // FORM SUBMIT (original - unchanged)
    // ============================================================
    document.getElementById('employeeForm')?.addEventListener('submit', function(e) {
        ...
        document.querySelectorAll('#expTableBody tr').forEach(tr => {
            if(!tr.querySelector('.e-del').checked) {
                expArr.push({ org: tr.querySelector('.e-org').value, add: tr.querySelector('.e-add').value, from: tr.querySelector('.e-from').value, to: tr.querySelector('.e-to').value, dept: tr.querySelector('.e-dept').value, desig: tr.querySelector('.e-desig').value, resp: tr.querySelector('.e-resp').value });
            }
        });
        ...
        const payload = {
            ...
            data: {
                ...
                empPhotoBase64: getVal('empPhotoBase64'), empSignBase64: getVal('empSignBase64')
            }
        };

        fetch(scriptURL, { method: 'POST', body: JSON.stringify(payload), headers: { "Content-Type": "application/json" } })
        .then(res => res.json())
        .then(data => {
            if(data.status === "Success") { alert(data.message); showView('module-employees-list'); syncWithDatabase(); }
    ...
    // ============================================================
    // EDUCATION TABLE ROWS (original - unchanged)
    // ============================================================
    ...
    function addQualRow(data = {}) {
        ...
        tr.innerHTML = `<td>${srNo}</td><td><select class="q-type">${cTypeOpts}</select></td><td><select class="q-name">${qNameOpts}</select></td><td><input type="text" class="q-inst" value="${data.inst || ''}"></td><td><input type="text" class="q-uni" value="${data.uni || ''}"></td><td><input type="number" class="q-dur" value="${data.dur || ''}"></td><td><input type="text" class="q-year" value="${data.year || ''}"></td><td><input type="text" class="q-perc" value="${data.perc || ''}"></td><td><input type="text" class="q-sub" value="${data.sub || ''}"></td><td><input type="text" class="q-ref" value="${data.ref || ''}" style="display:none;"></td><td><input type="checkbox" class="q-unv" ${data.unv ? 'checked' : ''} style="display:none;"></td><td><button type="button" class="btn btn-red" style="padding:4px 8px; font-size:11px;" onclick="this.closest('tr').remove()">🗑</button></td>`;
    ...
    function addExpRow(data = {}) {
        ...
        const srNo = tbody.children.length + 1; const tr = document.createElement('tr');
        tr.innerHTML = `<td class="e-sr">${srNo}</td><td><input type="text" class="e-org" value="${data.org || ''}"></td><td><input type="text" class="e-add" value="${data.add || ''}"></td><td><input type="text" class="e-desig" value="${data.desig || ''}"></td><td><input type="date" class="e-from" value="${data.from || ''}"></td><td><input type="date" class="e-to" value="${data.to || ''}"></td><td><input type="text" class="e-dept" value="${data.dept || ''}" style="display:none;"></td><td><input type="text" class="e-resp" value="${data.resp || ''}" style="display:none;"></td><td><input type="checkbox" class="e-del" title="Check to exclude on save"></td>`;
        tbody.appendChild(tr);
    }
    ...
    // ============================================================
    // INACTIVE EMPLOYEES LOGIC (original - unchanged)
    // ============================================================
    ...
    document.getElementById('btnMarkInactive')?.addEventListener('click', () => {
        ...
        if(confirm(`Are you sure you want to mark ${empId} as Inactive?`)) {
            fetch(scriptURL, { method: 'POST', body: JSON.stringify({ action: "inactiveEmployee", empId: empId, date: date, reason: reason }), headers: { "Content-Type": "application/json" } })
            .then(res => res.json())
            .then(data => {
    ...
    // ============================================================
    // MASTER SETUP (original - unchanged)
    // ============================================================
    ...
    function saveEmpSetupToDB() {
        ...
        fetch(scriptURL, { method: 'POST', body: JSON.stringify({ action: "saveEmpSetup", data: empSetup }), headers: { "Content-Type": "application/json" } })
        .then(res => res.json())
        .then(data => {
            if(data.status === "Success") {
                ...
                renderSetupDisplay(); populateSetupDropdowns();
            }
        })
    ...
    window.editES = function(cat, index) {
        const item = empSetup[cat][index];
        document.getElementById('esCategory').value = cat;
        document.getElementById('esEditIndex').value = index;
        document.getElementById('esValue').value = item;
        const btn = document.getElementById('btnSaveES'); btn.textContent = 'Update Entry'; btn.style.background = '#f39c12';
        document.getElementById('btnCancelESEdit').style.display = 'block';
    };

    window.deleteES = function(cat, index) {
        customConfirm("Are you sure you want to delete this option?", () => { empSetup[cat].splice(index, 1); saveEmpSetupToDB(); });
    };

    document.getElementById('btnCancelESEdit')?.addEventListener('click', () => {
        document.getElementById('empSetupForm').reset();
        document.getElementById('esEditIndex').value = "-1";
        const btn = document.getElementById('btnSaveES'); btn.textContent = 'Add Entry'; btn.style.background = '#5cb85c';
        document.getElementById('btnCancelESEdit').style.display = 'none';
    });
