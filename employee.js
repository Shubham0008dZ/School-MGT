document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!isSA) {
        if(!userRights.includes("HR_Add")) {
            let addBtn = document.getElementById('btn-open-add-emp');
            if(addBtn) addBtn.remove();
        }
        if(!userRights.includes("HR_Inactive")) {
            let inactNav = document.querySelector('.nav-btn[data-target="module-manage-employee"]');
            if(inactNav) inactNav.remove();
        }
        if(!userRights.includes("HR_Setup")) {
            let setupNav = document.querySelector('.nav-btn[data-target="module-emp-setup"]');
    ...
    function initData() {
        ...
        fetch(scriptURL).then(res => res.json()).then(res => {
            if(res.status === "Success") {
                ...
                populateSetupDropdowns(); renderSetupDisplay(); renderEmployeesTable(); populateInactiveDropdown(); renderInactiveEmployeesTable(); 
            }
        });
    }
    ...
    function renderEmployeesTable() {
        ...
        activeEmps.forEach((emp, idx) => {
            ...
            tbody.innerHTML += `<tr><td>${idx + 1}</td><td><img src="https://via.placeholder.com/30" style="border-radius:50%;"></td><td><b>${emp.empId}</b></td><td>${emp.empSalutation || ''}</td><td>${emp.empName}</td><td>${emp.empDept || '-'}</td><td>${emp.empDesig || '-'}</td><td>${emp.empMobile || '-'}</td><td>${emp.empBio || '-'}</td><td>${actionHtml}</td></tr>`;
        });
    }
    ...
    document.getElementById('btnMarkInactive').addEventListener('click', () => {
        ...
        if(confirm(`Are you sure you want to mark ${empId} as Inactive? They will be removed from the main list.`)) {
            fetch(scriptURL, { method: 'POST', body: JSON.stringify({ action: "inactiveEmployee", empId: empId, date: date, reason: reason }) }).then(res => res.json()).then(data => {
                if(data.status === "Success") { alert(data.message); document.getElementById('inactiveDate').value = ""; document.getElementById('inactiveReason').value = ""; initData(); }
            });
        }
    });
    ...
    function renderSetupDisplay() {
        ...
        Object.keys(empSetup).forEach(key => {
            if(empSetup[key] && empSetup[key].length > 0) {
                ...
                empSetup[key].forEach((item, index) => { html += `<div class="config-badge" style="display:inline-flex; align-items:center; background:#ecf0f1; border:1px solid #ccc; color:#333; margin:4px; padding:6px 12px; border-radius:3px;">${item}<span style="margin-left:10px; border-left:1px solid #aaa; padding-left:10px; cursor:pointer; color:#f39c12;" onclick="editES('${key}', ${index})" title="Edit">✏️</span><span style="margin-left:5px; cursor:pointer; color:#e74c3c;" onclick="deleteES('${key}', ${index})" title="Delete">🗑️</span></div>`; });
                html += `</div></div>`;
            }
        });
        disp.innerHTML = html;
    ...
    window.editES = function(category, index) {
        document.getElementById('esCategory').value = category; document.getElementById('esValue').value = empSetup[category][index]; document.getElementById('esEditCategory').value = category; document.getElementById('esEditIndex').value = index;
    ...
    window.deleteES = function(category, index) { if(confirm("Are you sure you want to delete this option?")) { empSetup[category].splice(index, 1); saveEmpSetupToDB(); } };

    document.getElementById('btnCancelESEdit').addEventListener('click', resetESForm);
    function resetESForm() {
        document.getElementById('empSetupForm').reset(); document.getElementById('esEditIndex').value = "-1"; document.getElementById('esEditCategory').value = "";
    ...
    document.getElementById('empSetupForm').addEventListener('submit', function(e) {
        e.preventDefault(); const cat = document.getElementById('esCategory').value; const val = document.getElementById('esValue').value.trim(); const editIndex = document.getElementById('esEditIndex').value; const editCategory = document.getElementById('esEditCategory').value;
        if(!empSetup[cat]) empSetup[cat] = [];
        if(editIndex !== "-1") { if(editCategory === cat) { empSetup[cat][editIndex] = val; } else { empSetup[editCategory].splice(editIndex, 1); empSetup[cat].push(val); } } else { if(!empSetup[cat].includes(val)) empSetup[cat].push(val); }
        saveEmpSetupToDB(); resetESForm();
    });
    ...
    function saveEmpSetupToDB() {
        ...
        fetch(scriptURL, { method: 'POST', body: JSON.stringify({ action: "saveEmpSetup", data: empSetup }) }).then(res => res.json()).then(data => { if(data.status === "Success") { initData(); } }).finally(() => { btn.textContent = oldText; btn.disabled = false; });
    }
