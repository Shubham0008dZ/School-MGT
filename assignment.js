// ============================================================================
// 1. GLOBAL UI FUNCTIONS (Modals & Custom Dropdowns)
// ============================================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadData() {
        ...
        if(cla) cla.innerHTML = '<p style="text-align:center; padding:20px; color:#777;">Fetching records... ⏳</p>';
        
        let calListArea = document.getElementById('calendarListArea');
        if(calListArea) calListArea.innerHTML = '<p style="text-align:center; padding:20px; color:#777;">Fetching calendar events... ⏳</p>';
    ...
    // ============================================================================
    // 4. CALENDAR EVENTS MODULE LOGIC
    // ============================================================================
    
    // Audience Toggle (Student vs Employee fields)
    ...
    if(evAudience) {
        evAudience.addEventListener('change', function() {
            if(this.value === "Students") { if(evStudentTgt) evStudentTgt.style.display = 'flex'; if(evEmpTgt) evEmpTgt.style.display = 'none'; }
            else if(this.value === "Employees") { if(evStudentTgt) evStudentTgt.style.display = 'none'; if(evEmpTgt) evEmpTgt.style.display = 'flex'; }
            else { if(evStudentTgt) evStudentTgt.style.display = 'flex'; if(evEmpTgt) evEmpTgt.style.display = 'flex'; }
        });
    }

    // Dynamic Student Checkboxes for Calendar
    function popCalStudents() {
        ...
        let cntEl = document.getElementById('evStudentCount'); if(cntEl) cntEl.innerText = "All";
    ...
    // Dynamic Employee Checkboxes for Calendar
    function popCalEmps() {
        ...
        let cntEl = document.getElementById('evEmpCount'); if(cntEl) cntEl.innerText = "All";

        if(!dept || dept === "All") return;

        let filteredEmps = allEmployees.filter(e => String(e.Department || e.empDept) === dept);
    ...
    // Bind Checking Logic and Count updates
    ...
    if(btnSaveEvent) {
        btnSaveEvent.addEventListener('click', function() {
            ...
            if(!evDate || !evTitle) { alert("Please fill Event Date and Title."); return; }
            ...
            let editId = document.getElementById('editEventId') ? document.getElementById('editEventId').value : "";
            ...
            .then(res => res.json()).then(data => {
                ...
                alert("Network Error during save."); console.error(err);
    ...
    window.renderCalendarList = function() {
        ...
        allEvents.slice().reverse().forEach(e => {
            let holiBadge = e.Is_Holiday === "Yes" ? '<span style="background:#e74c3c; color:white; padding:2px 5px; border-radius:3px; font-size:10px; margin-left:10px;">Std. Holiday</span>' : '';
            let empHoliBadge = e.Is_Emp_Holiday === "Yes" ? '<span style="background:#8e44ad; color:white; padding:2px 5px; border-radius:3px; font-size:10px; margin-left:5px;">Emp. Holiday</span>' : '';
            
            calList.innerHTML += `
                <div class="record-card" style="border-left:4px solid #f39c12; display:flex; justify-content:space-between; align-items:center;">
                    <div>
                        <div style="font-size:11px; color:#e67e22; margin-bottom:5px;"><b>${e.Date}</b> • Target: ${e.Audience} ${holiBadge} ${empHoliBadge}</div>
                    ...
                    <div style="display:flex; gap:5px; flex-direction:column;">
                        <button class="btn-action-edit" onclick="editEvent('${e.Event_ID}')">✏️ Edit</button>
                        <button class="btn-action-del" onclick="deleteEvent('${e.Event_ID}')">🗑️ Del</button>
                    </div>
                </div>
            `;
        });
    ...
    window.editEvent = function(eventId) {
        ...
        if(ev.Date) {
            ...
            if(parts.length === 3) document.getElementById('evDate').value = `${parts[2]}-${parts[1]}-${parts[0]}`;
        }
        
        document.getElementById('evTitle').value = ev.Title;
        document.getElementById('evDesc').value = ev.Description || "";
        document.getElementById('evHoliday').checked = (ev.Is_Holiday === "Yes");
        if(document.getElementById('evEmpHoliday')) document.getElementById('evEmpHoliday').checked = (ev.Is_Emp_Holiday === "Yes");
        ...
        setTimeout(() => { document.getElementById('evSection').value = ev.Target_Section || "All"; document.getElementById('evSection').dispatchEvent(new Event('change')); }, 100);
        ...
        setTimeout(() => {
            try {
                ...
                if(stuTgt[0] === "All") { 
                    let sAllBox = document.querySelector('.ev-stu-chk[value="All"]'); if(sAllBox) sAllBox.checked = true; 
                    let sCount = document.getElementById('evStudentCount'); if(sCount) sCount.innerText = "All"; 
                } else {
                    let sAllBox = document.querySelector('.ev-stu-chk[value="All"]'); if(sAllBox) sAllBox.checked = false;
                    document.querySelectorAll('.ev-stu-chk').forEach(chk => { if(stuTgt.includes(chk.value)) chk.checked = true; });
                    let sCount = document.getElementById('evStudentCount'); if(sCount) sCount.innerText = stuTgt.length;
                }
                
                let empTgt = JSON.parse(ev.Target_Emp || '["All"]');
                if(empTgt[0] === "All") { 
                    let eAllBox = document.querySelector('.ev-emp-chk[value="All"]'); if(eAllBox) eAllBox.checked = true; 
                    let eCount = document.getElementById('evEmpCount'); if(eCount) eCount.innerText = "All"; 
                } else {
                    let eAllBox = document.querySelector('.ev-emp-chk[value="All"]'); if(eAllBox) eAllBox.checked = false;
                    document.querySelectorAll('.ev-emp-chk').forEach(chk => { if(empTgt.includes(chk.value)) chk.checked = true; });
                    let eCount = document.getElementById('evEmpCount'); if(eCount) eCount.innerText = empTgt.length;
                }
            } catch(e){ console.error("Parse targeting error", e); }
        }, 300);
    ...
    window.deleteEvent = function(eventId) {
        customConfirm("Are you sure you want to delete this event?", () => {
            fetch(scriptURL, { method: 'POST', body: JSON.stringify({ action: "deleteEvent", eventId: eventId }), redirect: "follow", headers: { "Content-Type": "text/plain;charset=utf-8" } })
            .then(res => res.json()).then(data => {
                if(data.status === "Success") { alert("Event Deleted!"); loadData(); } 
                else { alert("Error: " + data.message); }
            });
        });
    };
    ...
    // ============================================================================
    // 5. COMMUNICATION / ASSIGNMENT MODULE LOGIC
    // ============================================================================
    
    document.getElementById('btnOpenAddForm')?.addEventListener('click', () => {
        document.querySelectorAll('.app-module').forEach(m => { m.classList.remove('active-module'); m.style.display = 'none'; });
        let addMod = document.getElementById('module-add-hw'); 
        if(addMod) { addMod.classList.add('active-module'); addMod.style.display = 'block'; }
        
        let dynTitle = document.getElementById('formDynamicTitle'); if(dynTitle) dynTitle.innerText = currentCategory.toUpperCase();
        let editFld = document.getElementById('editAssignmentId'); if(editFld) editFld.value = ""; 
        let saveBtn = document.getElementById('btnSaveHw'); if(saveBtn) saveBtn.innerText = "Save Entry";
        
        let defaultType = currentCategory; if(currentCategory === "Homework") defaultType = "Homework"; 
        
        let hwTypeEl = document.getElementById('hwType'); if(hwTypeEl) hwTypeEl.value = defaultType; 
        let hwDateEl = document.getElementById('hwDate'); if(hwDateEl) hwDateEl.value = new Date().toISOString().split('T')[0];

        let targetAll = document.querySelector('input[name="targetType"][value="All"]');
    ...
    document.querySelectorAll('input[name="targetType"]').forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            if(this.value === "Specific") { if(specCont) specCont.style.display = 'block'; populateCommStudentCheckboxes(); } 
            else { if(specCont) specCont.style.display = 'none'; }
        });
    });

    document.getElementById('hwClass')?.addEventListener('change', populateCommStudentCheckboxes);
    document.getElementById('hwSection')?.addEventListener('change', populateCommStudentCheckboxes);
    ...
    window.editComm = function(assignId) {
        ...
        if(formMod) { formMod.classList.add('active-module'); formMod.style.display = 'block'; }
        
        let dynTitle = document.getElementById('formDynamicTitle'); if(dynTitle) dynTitle.innerText = "EDIT " + a.Type.toUpperCase();
        ...
        let hwClassEl = document.getElementById('hwClass'); if(hwClassEl) hwClassEl.value = a.Class;
        ...
        let hwSub = document.getElementById('hwSubject'); if(hwSub) hwSub.value = a.Subject || "";
        let hwType = document.getElementById('hwType'); if(hwType) hwType.value = a.Type;
        let hwName = document.getElementById('hwName'); if(hwName) hwName.value = a.Name;
        let hwDesc = document.getElementById('hwDesc'); if(hwDesc) hwDesc.value = a.Description;
        let hwSubReq = document.getElementById('hwSubmissionReq'); if(hwSubReq) hwSubReq.checked = (a.Submission_Required === "Yes");
        let hwB64 = document.getElementById('hwBase64'); if(hwB64) hwB64.value = a.Attachment_Base64 || "";
        let hwFileN = document.getElementById('hwFileName'); if(hwFileN) hwFileN.innerText = a.Attachment_Base64 ? "Existing Attachment Loaded" : "No file selected";
        
        try {
            let tg = JSON.parse(a.Target_Students || '["All"]');
            let tAll = document.querySelector('input[name="targetType"][value="All"]');
            let tSpec = document.querySelector('input[name="targetType"][value="Specific"]');
        ...
        try {
            ...
            if(a.Type === "Achievement") {
                let ag = document.getElementById('achGroup'); if(ag) ag.value = meta.group || "";
                let al = document.getElementById('achLevel'); if(al) al.value = meta.level || "";
                let ar = document.getElementById('achRank'); if(ar) ar.value = meta.rank || "";
                let av = document.getElementById('achVenue'); if(av) av.value = meta.venue || "";
            } else if (a.Type === "Remarks") {
                let rc = document.getElementById('remCat'); if(rc) rc.value = meta.category || "";
                let rt = document.getElementById('remType'); if(rt) rt.value = meta.type || "";
            }
        } catch(e){}

        let sBtn = document.getElementById('btnSaveHw'); if(sBtn) sBtn.innerText = "Update Entry";
    ...
    if(hwFileInput) {
        hwFileInput.addEventListener('change', function() {
            ...
            if(file) { 
                ...
                reader.onload = function(e) { if(b64El) b64El.value = e.target.result; }; 
    ...
    if(hwForm) {
        hwForm.addEventListener('submit', function(e) {
            ...
            let targetTypeEl = document.querySelector('input[name="targetType"]:checked');
            ...
            if(targetType === "Specific") {
                ...
                if(targetArray.length === 0) {
                    ...
                    if(btn) { btn.innerText = "Save Entry"; btn.disabled = false; }
                    return;
                }
            }
            ...
            if(type === "Achievement") {
                metaData = {
                    group: document.getElementById('achGroup') ? document.getElementById('achGroup').value : "",
                    level: document.getElementById('achLevel') ? document.getElementById('achLevel').value : "",
                    rank: document.getElementById('achRank') ? document.getElementById('achRank').value : "",
                    venue: document.getElementById('achVenue') ? document.getElementById('achVenue').value : ""
                };
            } else if (type === "Remarks") {
                metaData = {
                    category: document.getElementById('remCat') ? document.getElementById('remCat').value : "",
                    type: document.getElementById('remType') ? document.getElementById('remType').value : ""
                };
            }
            ...
            if(extraValidationPassed) {
                const payload = {
                    ...
                    data: { 
                        assignmentId: editId,
                        acadYear: document.getElementById('acadYear') ? document.getElementById('acadYear').value : "", 
                        date: formatToDDMMYYYY(document.getElementById('hwDate') ? document.getElementById('hwDate').value : ""), 
                        class: document.getElementById('hwClass') ? document.getElementById('hwClass').value : "", 
                        section: document.getElementById('hwSection') ? document.getElementById('hwSection').value : "", 
                        subject: document.getElementById('hwSubject') ? document.getElementById('hwSubject').value : "", 
                        type: type, 
                        name: document.getElementById('hwName') ? document.getElementById('hwName').value : "", 
                        studentWise: false, 
                        description: document.getElementById('hwDesc') ? document.getElementById('hwDesc').value : "", 
                        attachmentBase64: document.getElementById('hwBase64') ? document.getElementById('hwBase64').value : "", 
                        submissionReq: document.getElementById('hwSubmissionReq') ? document.getElementById('hwSubmissionReq').checked : false, 
                        createdBy: activeUser.empId,
                ...
                .then(res => res.json()).then(data => {
                    if(data.status === "Success") {
                        ...
                        let dEl = document.getElementById('hwDate'); if(dEl) dEl.value = new Date().toISOString().split('T')[0]; 
                        let fEl = document.getElementById('hwFileName'); if(fEl) fEl.innerText = "No file selected"; 
                        let bEl = document.getElementById('hwBase64'); if(bEl) bEl.value = "";
