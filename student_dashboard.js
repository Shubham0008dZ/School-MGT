...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(scriptURL, { redirect: "follow" }).then(res => res.json()).then(data => {
        if(data.status === "Success") {
            ...
            try { processEvents(data.events); } catch(e) { console.error("Event error:", e); }
            try { processFeeLedger(data.receipts, data.setup, data.feeHeads); } catch(e) { console.error("Fee error:", e); }
            try { renderFeed(data.assignments, data.submissions); } catch(e) { console.error("Feed error:", e); }
            try { processAttendance(data.attendance); } catch(e) { console.error("Attendance error:", e); }
            
        } else {
            console.error("Backend Error:", data.message);
            let feedCont = document.getElementById('mainFeedContainer');
            if(feedCont) feedCont.innerHTML = `<p style="color:red; text-align:center;">Failed to fetch data: ${data.message}</p>`;
        }
    }).catch(err => {
...
function renderFeed(assignments, submissions) {
    ...
    let myItems = assignments.filter(a => {
        ...
        if(currentFeedFilter === "Circulars") typeMatch = ["Circular", "Notice", "News"].includes(a.Type);
                    ...
                    if(fileInp) {
                        fileInp.addEventListener('change', function() { 
                            if(this.files[0]) { 
                                ...
                                r.onload = function(e) { document.getElementById('stuAnsBase64').value = e.target.result; }; 
