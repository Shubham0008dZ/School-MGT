...
// ============================================================================
const scriptURL =
  "https://script.google.com/macros/s/AKfycbyDv3nOs6E9OQOSXBywbYHJPpl_V8frIegpSmTCZFRlsh1xis6iS-SMZxEWxIqJ6s-aEw/exec";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (document.getElementById("dashClass"))
    document.getElementById("dashClass").innerText = sClass;

  if (
    document.getElementById("dashAvatarBox") &&
    studentData.studentPhotoBase64 &&
    studentData.studentPhotoBase64.startsWith("data:image")
  ) {
    document.getElementById("dashAvatarBox").innerHTML =
      `<img src="${studentData.studentPhotoBase64}" alt="Profile" style="width:100%;height:100%;object-fit:cover;">`;
  }
  ...
  fetch(scriptURL, { redirect: "follow" })
    ...
    .then((data) => {
      if (data.status === "Success") {
        ...
        try {
          renderFeed(data.assignments, data.submissions);
        } catch (e) {
        ...
        if (feedCont)
          feedCont.innerHTML = `<p style="color:red; text-align:center;">Failed to fetch data: ${data.message}</p>`;
      }
    })
    .catch((err) => {
  ...
  document.getElementById("bigCalPrevMonth")?.addEventListener("click", () => {
    currDate.setMonth(currDate.getMonth() - 1);
    renderCalendar();
  });
  document.getElementById("bigCalNextMonth")?.addEventListener("click", () => {
    currDate.setMonth(currDate.getMonth() + 1);
    renderCalendar();
  });
});
...
// ============================================================================
// EVENTS PROCESSING (For Calendar & Right Sidebar)
// ============================================================================
function processEvents(events) {
  if (!events) return;

  let sClassRaw = studentData.studentClass
    ? studentData.studentClass.split("(")[0].trim()
    : "";
  ...
  let validEvents = events.filter((e) => {
    ...
    if (
      e.Target_Class !== "All" &&
      String(e.Target_Class) !== String(sClassRaw)
    )
      return false;
    return true;
  });
  ...
  if (validEvents.length === 0) {
    if (evtCont)
      evtCont.innerHTML =
        '<div class="empty-illustration"><span>📭</span><p>No upcoming events.</p></div>';
  } else {
...
// ============================================================================
// UNIFIED TIMELINE FEED LOGIC
// ============================================================================
window.filterFeed = function (filterVal) {
  switchView("view-dashboard");

  document
    .querySelectorAll(".comm-pill")
    .forEach((t) => t.classList.remove("active-pill"));
  if (event && event.target && event.target.classList.contains("comm-pill")) {
    event.target.classList.add("active-pill");
  } else {
    // If triggered from sidebar, select corresponding pill if it exists
    let pill = document.querySelector(
      `.comm-pill[onclick="filterFeed('${filterVal}')"]`,
    );
...
function renderFeed(assignments, submissions) {
  ...
  if (!assignments || assignments.length === 0) {
    feedCont.innerHTML =
      '<p style="text-align:center; padding:30px; color:#94a3b8;">No updates found.</p>';
    return;
  }
  ...
  let myItems = assignments
    .filter((a) => {
      let matchClass =
        String(a.Class) === String(sClassRaw) ||
        String(a.Class) === "All" ||
        !a.Class;
      ...
      let matchTarget =
        targets.includes("All") || targets.includes(String(studentData.regNo));

      // Pill Filters
      let typeMatch = true;
      if (currentFeedFilter === "Homework")
        typeMatch = ["Homework", "Classwork", "Assignment", "Project"].includes(
          a.Type,
        );
      if (currentFeedFilter === "Circulars")
        typeMatch = ["Circular", "Notice", "News"].includes(a.Type);
      if (currentFeedFilter === "Achievements")
        typeMatch = ["Achievement"].includes(a.Type);
      if (currentFeedFilter === "Remarks")
        typeMatch = ["Remarks", "Feedback"].includes(a.Type);
  ...
  if (myItems.length === 0) {
    feedCont.innerHTML =
      '<div class="empty-illustration"><span>📭</span><p>No recent updates in this category.</p></div>';
    return;
  }
  ...
  myItems.forEach((a) => {
    ...
    let iconHtml = "📢";
    let themeClass = "circ";
    if (["Homework", "Classwork", "Assignment", "Project"].includes(type)) {
      iconHtml = "📚";
      themeClass = "hw";
    } else if (type === "Achievement") {
      iconHtml = "🏆";
      themeClass = "ach";
    } else if (["Remarks", "Feedback"].includes(type)) {
      iconHtml = "📝";
      themeClass = "rem";
    } else if (type === "PTM") {
      iconHtml = "🤝";
    ...
    let subjectHtml = a.Subject
      ? `<span style="font-size:11px; font-weight:bold; color:#64748b; text-transform:uppercase;">${a.Subject}</span>`
      : "";
    ...
    try {
      if (a.Metadata_JSON && a.Metadata_JSON.trim() !== "") {
        ...
        if (type === "Achievement") {
          if (meta.rank)
            metaHtml += `<span style="font-size:11px; background:#fef3c7; color:#b45309; padding:2px 6px; border-radius:4px; font-weight:bold; margin-right:5px;">Rank: ${meta.rank}</span>`;
          if (meta.level)
            metaHtml += `<span style="font-size:11px; background:#f1f5f9; color:#475569; padding:2px 6px; border-radius:4px; font-weight:bold; margin-right:5px;">Level: ${meta.level}</span>`;
          if (meta.venue)
            metaHtml += `<span style="font-size:11px; color:#64748b; font-weight:bold;">Venue: ${meta.venue}</span>`;
    ...
    if (metaHtml !== "")
      metaHtml = `<div style="margin-bottom:12px;">${metaHtml}</div>`;
    ...
    let hwActionHtml = "";
    if (themeClass === "hw") {
      let mySub = (submissions || []).find(
        (s) =>
          String(s.Assignment_ID) === String(a.Assignment_ID) &&
          String(s.Reg_No) === String(studentData.regNo),
      );
      ...
      let aDataStr = JSON.stringify(a)
        .replace(/'/g, "&#39;")
        .replace(/"/g, "&quot;");
      let subDataStr = mySub
        ? JSON.stringify(mySub).replace(/'/g, "&#39;").replace(/"/g, "&quot;")
        : "null";
      ...
      if (attachHtml)
        hwActionHtml = `<div style="margin-top:10px;">${attachHtml}</div>`;
      attachHtml = "";
    }
...
// ============================================================================
// MODAL INTERACTION: EYE BUTTON LOGIC
// ============================================================================
...
window.openHwModal = function (hwData, subData) {
  ...
  if (descEl)
    descEl.innerText = hwData.Description || "No description attached.";
  ...
  if (subArea) {
    ...
    if (hwData.Submission_Required === "Yes") {
      ...
      if (formattedDueDate) {
        let parts = formattedDueDate.split("-"); // Expected DD-MM-YYYY
        if (parts.length === 3) {
          let dueDateObj = new Date(
            `${parts[2]}-${parts[1]}-${parts[0]}T23:59:59`,
          );
      ...
      if (
        isPastDue &&
        (!subData || (!subData.Marks && subData.Status !== "Submitted"))
      ) {
        // PAST DUE DATE AND NOT SUBMITTED
        subArea.innerHTML = `
          ...
          if (fileInp) {
            fileInp.addEventListener("change", function () {
              if (this.files[0]) {
                ...
                r.onload = function (e) {
                  document.getElementById("stuAnsBase64").value =
                    e.target.result;
                };
...
window.submitHwToDb = function () {
  ...
  const payload = {
    ...
    data: {
      ...
      answerText: document.getElementById("stuAnsText")
        ? document.getElementById("stuAnsText").value
        : "",
      attachmentBase64: document.getElementById("stuAnsBase64")
        ? document.getElementById("stuAnsBase64").value
        : "",
    },
  };
...
function processFeeLedger(receipts, setupData, feeHeads) {
  ...
  if (studentData.studentClass && setupData && setupData.classes) {
    let cSetup = setupData.classes.find(
      (c) =>
        `${c.name} (${c.section})` === studentData.studentClass ||
        c.name === studentData.studentClass,
    );
    if (cSetup && cSetup.fee) classFeeAmount = parseFloat(cSetup.fee);
  }
  let paidMap = {};
  let myReceipts = (receipts || []).filter(
    (r) => String(r.Reg_No) === String(studentData.regNo),
  );
  myReceipts.forEach((r) => {
    try {
      let rawHeads = String(r.Paid_Heads || "").trim();
      if (rawHeads !== "" && rawHeads.startsWith("[")) {
        JSON.parse(rawHeads).forEach((d) => {
          paidMap[d.head + "_" + d.period] =
            (paidMap[d.head + "_" + d.period] || 0) + parseFloat(d.paid || 0);
        });
      }
    } catch (e) {}
  });
  ...
  (feeHeads || []).forEach((fh) => {
    if (fh.Frequency === "Annually" || fh.Frequency === "One Time (Annually)") {
      ...
      let pd =
        paidMap[fh.Head_Name + "_" + fh.Frequency] ||
        paidMap[fh.Head_Name + "_Annually"] ||
        0;
      let bal = amt - pd;
      totalDue += amt;
      totalPaid += pd;
  ...
  if (document.getElementById("feeTotApplicable"))
    document.getElementById("feeTotApplicable").innerText =
      "₹" + totalDue.toFixed(2);
  if (document.getElementById("feeTotPaid"))
    document.getElementById("feeTotPaid").innerText =
      "₹" + totalPaid.toFixed(2);
  if (document.getElementById("feeTotDue"))
    document.getElementById("feeTotDue").innerText = "₹" + netDue.toFixed(2);
...
// ============================================================================
// 3. ATTENDANCE & CALENDAR MODULE (WITH EVENT TOOLTIPS)
// ============================================================================
function processAttendance(records) {
  ...
  if (document.getElementById("attTotalP"))
    document.getElementById("attTotalP").innerText = totP;
  if (document.getElementById("attTotalA"))
    document.getElementById("attTotalA").innerText = totA;
  if (document.getElementById("attTotalL"))
...
function renderCalendar() {
  ...
  if (calT1)
    calT1.innerText = currDate.toLocaleDateString("en-GB", { month: "short" });
  if (calT2)
    calT2.innerText = currDate.toLocaleDateString("en-GB", {
      month: "long",
      year: "numeric",
    });
  ...
  const smallCalGrid = document.getElementById("smallCalGrid");
  ...
  for (let i = 0; i < firstDay; i++) {
    htmlBig += `<div class="cal-cell" style="background:none; border:none;"></div>`;
    htmlSmall += `<div class="cal-cell small"></div>`;
  }
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    if (evt && evt.isHoliday) {
      ...
      smallDot = `<div class="cal-dot" style="background:#27ae60;"></div>`;
      ...
      dotHtml = `<div class="dot l"></div>`;
      smallDot = `<div class="cal-dot" style="background:#f59e0b;"></div>`;
      hoverCardContent = `<strong style="color:#f59e0b;">Leave / Late</strong>`;
    }

    if (evt && !evt.isHoliday) {
      hoverCardContent += hoverCardContent
        ? `<hr style="margin:5px 0; border:1px solid #334155;">`
        : ``;
      hoverCardContent += `<strong>📌 ${evt.title}</strong>`;
      smallDot = `<div class="cal-dot" style="background:#3b82f6;"></div>`;
    }

    let hoverHtml = hoverCardContent
      ? `<div class="hover-card">${hoverCardContent}</div>`
      : "";
    let isToday =
